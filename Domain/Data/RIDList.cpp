...
Domain::Data::RIDList::RIDList() : vector<RecordID>() {}

// Performs an AND operation on two sorted RecordLists
Domain::Data::RIDList Domain::Data::RIDList::operator&(RIDList& other) {
	RIDList results;
...
string Domain::Data::RIDList::Inspect() {
	string out;
	
	out += "[";
	for(size_t i = 0; i < vector<RecordID>::size(); i++) {
		out += boost::lexical_cast<string>(vector<RecordID>::at(i));
